...
def operationConsistency(){
    ...
    dynamicPage(pageProperties) {
        section("power consistency")
        {
            ...
            //     log.warn "pw = $pw must be off loaded from settings"
...
def dimmerHandler(evt){
    ...
    if (!atomicState.paused) {
        ...
        log.warn "new dimmer level is $evt.value method = $method && setpointSentByApp = $atomicState.setpointSentByApp"
...
def pushableButtonHandler(evt){
    if (!atomicState.paused) {
        ...
        if (evt.name == "pushed") {
            ...
            log.warn debug_from_list(warning)
...
def mainloop(source = "UNDEFINED"){
    ...
    log.warn "MAINLOOP INITIALIZATION block"
    try {
        ...
        try {
            if (neededThermostats[0]) {
                ...
                if (enabletrace) log.warn "neededThermostats => $neededThermostats"
    ...
    log.warn "INITIALIZATION took: ${(now() - blocktime)/1000} seconds"
    ...
    log.warn "LOG TIMER CHECK took: ${(now() - blocktime)/1000} seconds"

    /********************** VERIFY HEATPUMP AND POWER USAGE CONDITIONS (HEATER OR COOLER)*************************/

    log.warn "powerManagement block"
    blocktime = now()
    try {
        if (enablewarning) log.warn "neededThermostats ================ $neededThermostats";
    ...
    log.warn "powerManagement block took: ${(now() - blocktime)/1000} seconds"


    // VIRTUAL THERMOSTAT
    log.warn "VIRTUAL THERMOSTAT block"
    blocktime = now()
    try {
    ...
    log.warn "VIRTUAL THERMOSTAT block took: ${(now() - blocktime)/1000} seconds"
...
def powerManagement(inside, outside, need, target, cmd, contactClosed, doorsContactsAreOpen, motionActive, heatpumpConditionsTrue, dontSendThermostatModeCmd, currSP, neededThermostats, thermModes, opStateOk){
    ...
    if (!atomicState.override) {        
        ...
        try {
            ...
            if (coolerNotEfficientEnough && need == "cool") if (enablewarning) log.warn format_text("$cooler not efficient enough, turning on $thermostat", "red", "white")
        ...
        try {
            ...
            if (!atomicState.setPointOverride && (thermostatModeNotOk || forceCommand || (overrideThermostatModeCheckBeforeSendingCmd && atomicState.dontChekcThermostatStateCount < atomicState.reCheckStateCount)) && contactClosed) {
                ...
                if (enablewarning || thermostatModeNotOk) {
                    log.warn "thermostatModeNotOk ===> $thermostatModeNotOk"
                    log.warn "overrideThermostatModeCheckBeforeSendingCmd ===> $overrideThermostatModeCheckBeforeSendingCmd"
                    log.warn "atomicState.dontChekcThermostatStateCount < atomicState.reCheckStateCount ===> $atomicState.dontChekcThermostatStateCount < atomicState.reCheckStateCount"
                }
                ...
                if ((currSpNotOk || thermostatModeNotOk) && !thermTempDiscrepancy && contactClosed) {
                    ...
                    if (currSpNotOk || thermostatModeNotOk) {
                        ...
                        if (need == "off" && ignoreMode) {
                            ...
                            log.debug "Setting $thermostat to $need (5rrgh4klt5ui)"
...
def powerConsistency(need, target, inside, cmd, contactClosed, doorsContactsAreOpen, motionActive, thermModes, operatingStateOk, currentOperatingNeed){
    ...
    try {
        if (pw) {
            ...
            // timeIsUp = true
            if (need != "off" && !ignoreMode && timeIsUp && pwLow) {
                    ...
                    if (!operatingStateOk) {
                        ...
                        try {
                            for (device in faulty_devices) {
                                ...
                                log.warn("TRYING TO SET $device to $need")
    ...
    try {
        ...
        try {
            ...
            if (enablewarning || swamp) {
                log.warn "needCool => $needCool"
                log.warn "swamp => $swamp"
            }
    ...
    try {
        ...
        else   // POWER SAVING MODE OR NO MOTION OR CONTACTS OPEN     
        {
            ...
            if (inside > criticalhot || fancirculateAllways) {
                log.warn format_text("${fancirculateAllways ? "fan set to always be on" : "POWER SAVING MODE EXPCETION: TOO HOT!(${ cause } criticalhot = ${ criticalhot })"}", "black", "red")
                need0 = "Cool"
                need1 = "cool"
                ...
                if (enablewarning) log.warn format_text("POWER SAVING MODE EXPCETION: TOO COLD! ($cause criticalcold = $criticalcold)", "white", "blue")
                need0 = "Heat"
                need1 = "heat"
            }
            else {
...
def get_inside_temperature(){
    ...
    if (sensor) {
        ...
        for (i = 0; i < s; i++) {
            ...
            if (events == 0) {
                if (enablewarning) log.warn format_text("$device hasn't returned any event in the last 72 hours!", "white", "red")
                if (sensor.size() > 1) {
            ...
            if (device.hasCapability("Battery")) {
                ...
                {
                    if (enablewarning) log.warn format_text("WARNING! ${device}'s BATTERY LEVEL IS LOW (${batteryValue}%)", "white", "red")
...
def set_multiple_thermostats_mode(mode, origin, safeValue){
    ...
    log.warn "--                                               set_multiple_thermostats_mode                                                --"

    boolean forceCommand = origin == "checkthermstate force command"
...
def set_thermostat_mode(t, mode, origin){
    log.warn "set_thermostat_mode called from $origin"
    try {
        if (t.currentValue("thermostatMode") != mode || origin in ["checkthermstate force command", "remainsOffCmd"]) {
            ...
            if (autoOverride && t.currentValue("thermostatMode") == "auto") {
                log.warn "$t is in auto mode - command to set to $mode ignored..."
            }
            else {
...
def turn_off_thermostats(need, inside, thermModes) {
    ...
    log.warn "--                                                         turn_off_thermostats                                                         --"


    if ((atomicState.lastNeed == "heat" && inside < criticalcold) || (atomicState.lastNeed == "cool" && inside > criticalhot)) {
        log.warn "not turning off thermostat because inside temp is ${inside < criticalcold ? 'too low' : 'too hot'}"
        return
    }
    else {
        ...
        if (simpleModeIsActive() && dontTurnOffinNightMode) {
            log.warn "NOT Turning off thermostats because in simple mode and dontTurnOffinNightMode is true"
            return
        }
                ...
                if (testMotionBattery) {
                    ...
                    if (devicesWithLowBattery.size() != 0) {
                        ...
                        if (devicesWithLowBattery.size() == motionSensors.size()) {
                            log.warn format_text(m, "white", "red")
