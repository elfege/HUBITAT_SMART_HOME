...
def dimmerHandler(evt){
    ...
    if (!atomicState.paused) {
        ...
        if (enabledebug) log.debug "new dimmer level is $evt.value method = $method && setpointSentByApp = $atomicState.setpointSentByApp"
...
def windowsHandler(evt){
    if (!atomicState.paused) {
        ...
        if (description) log.info "$evt.device is $evt.value"
        boolean doorsContactsAreOpen = doorsContactsAreOpen()
...
def mainloop(source = "UNKNOWN"){
    ...
    try {
        ...
        if (atomicState.busy) {
            log.debug "$app.label is busy... skipping. Duration: ${now() - start} ms"
            // prevent stacking
            return
        }
        ...
        long s = now()
        if (enabledebug) log.debug "Checking contactsAreOpen"
        boolean contactClosed = !contactsAreOpen()
        if (enabledebug) log.debug "Result of contactsAreOpen: $contactClosed execution time: ${now() -s} ms"
        if (enabledebug) s = now()
        if (enabledebug) log.debug "Checking simpleModeIsActive"
        boolean simpleModeActive = simpleModeIsActive()
        if (enabledebug) log.debug "Result of simpleModeIsActive: $simpleModeActive execution time: ${now() -s} ms"
        if (enabledebug) s = now()
        if (enabledebug) log.debug "Checking Active"
        boolean motionActive = Active()
        if (enabledebug) log.debug "Result of Active: $motionActive execution time: ${now() -s} ms"
        if (enabledebug) s = now()
        if (enabledebug) log.debug "Checking doorsContactsAreOpen"
        boolean doorsContactsAreOpen = doorsContactsAreOpen()
        if (enabledebug) log.debug "Result of doorsContactsAreOpen: $doorsContactsAreOpen execution time: ${now() -s} ms"
        if (enabledebug) s = now()
        if (enabledebug) log.debug "Getting target from getTarget"
        def target = getTarget(simpleModeActive)
        if (enabledebug) log.debug "Result of getTarget: $target execution time: ${now() -s} ms"
        if (enabledebug) s = now()
        if (enabledebug) log.debug "Getting inside temperature from getInsideTemp"
        def inside = getInsideTemp()
        if (enabledebug) log.debug "Result of getInsideTemp: $inside execution time: ${now() -s} ms"
        if (enabledebug) s = now()
        if (enabledebug) log.debug "Getting outside temperature from outsideTemp"
        def outside = outsideTemp.currentValue("temperature")
        if (enabledebug) log.debug "Result of outsideTemp: $outside execution time: ${now() -s} ms"
        if (enabledebug) s = now()
        if (enabledebug) log.debug "Calculating need data from getNeed"
        def needData = getNeed(target, simpleModeActive, inside, motionActive, "mainloop")
        if (enabledebug) log.debug "Result of getNeed: $needData execution time: ${now() -s} ms"
        if (enabledebug) s = now()
        if (enabledebug) log.debug "Getting need"
        def need = needData[1]
        if (enabledebug) log.debug "need = $need execution time: ${now() -s} ms"
        if (enabledebug) s = now()
        if (enabledebug) log.debug "Getting needed thermostats from get_needed_thermosats"
        def neededThermostats = get_needed_thermosats(need)
        if (enabledebug) log.debug "Result of get_needed_thermosats: $neededThermostats execution time: ${now() -s} ms"
        if (enabledebug) s = now()
        if (enabledebug) log.debug "Checking thermostat mode"
        def thermMode = [thermostat?.currentValue("thermostatMode")]
        if (enabledebug) log.debug "Result of thermostat mode check: $thermMode execution time: ${now() -s} ms"
        if (enabledebug) s = now()
        if (enabledebug) log.debug "Creating cmd string"
        def cmd = "set" + "${needData[0]}" + "ingSetpoint" // "Cool" or "Heat" with a capital letter
        if (enabledebug) log.debug "Cmd string: $cmd execution time: ${now() -s} ms"
        if (enabledebug) s = now()
        if (enabledebug) log.debug "Checking doNotUseMain condition"
        boolean doNotUseMain = doNotUseMainThermostatInCertainModes && location.mode in altThermostatORheaterOnlyModes
        if (enabledebug) log.debug "Result of doNotUseMain condition: $doNotUseMain execution time: ${now() -s} ms"
        if (enabledebug) s = now()
        if (enabledebug) log.debug "Checking heatpumpConditions"
        ...
        if (enabledebug) log.debug "Result of heatpumpConditions check: $heatpumpConditionsTrue execution time: ${now() -s} ms"
        if (enabledebug) s = now()
        if (enabledebug) log.debug "Checking ignoreTherMode condition"
        boolean ignoreTherMode = (dontSetThermModes && simpleModeIsActive()) || ignoreMode
        if (enabledebug) log.debug "Result of ignoreTherMode condition: $ignoreTherMode execution time: ${now() -s} ms"
        if (enabledebug) s = now()
        if (enabledebug) log.debug "Getting current setpoint"
        def currSP = [thermostat?.currentValue("thermostatSetpoint").toInteger()]
        if (enabledebug) log.debug "Current setpoint: $currSP execution time: ${now() -s} ms"
        if (enabledebug) s = now()
        if (enabledebug) log.debug "Adding setpoints of needed thermostats"
        currSP += neededThermostats?.collect{ it.currentValue("thermostatSetpoint").toInteger() }
        if (enabledebug) log.debug "Updated setpoints: $currSP execution time: ${now() -s} ms"
        ...
        if (differentiateThermostatsHeatCool) {
            ...
            if (enabledebug) log.debug "thermMode collection: $thermMode execution time: ${now() -s} ms"
        }
        ...
        try {
            powerManagement(inside, need, target, cmd, contactClosed, doorsContactsAreOpen, motionActive, heatpumpConditionsTrue, ignoreTherMode, currSP)
        } catch (Exception e) {
            log.error "powerManagement() => $e"
        }
        try {
            fanCirculateManagement(need, target, contactClosed, motionActive)
        } catch (Exception e) {
...
def powerManagement(inside, need, target, cmd, contactClosed, doorsContactsAreOpen, motionActive, heatpumpConditionsTrue, ignoreTherMode, currSP){
    if (pw) {
        if (enabledebug) log.debug "$pw power meter returns ${pw?.currentValue("power")} Watts"
    }
    if (!atomicState.override) {
        ...
        try {
            ...
            // // The stack trace is now stored in the StringWriter
            // // Convert it to a String
            // def stackTraceAsString = sw.toString()

            // // log the exception string or process it as needed
            // log.error "checkHeatPump => $stackTraceAsString"
        }


        // POWER USAGE CONSISTENCY TEST
        try {
            powerConsistency(need, target, cmd, contactClosed, doorsContactsAreOpen, motionActive)
        }
        catch (Exception e) {
            ...
            // log.error "powerConsistency => $stackTraceAsString"
        }
        ...
        try {
            ...
            boolean OperatingStateOk = contactClosed && !doorsContactsAreOpen ? currentOperatingState in [currentOperatingNeed, "fanCirculate"] : true
            ...
            if (differentiateThermostatsHeatCool) {
                ...
                if (enabledebug) log.debug "currentOperatingNeed => $currentOperatingNeed"
                if (logwarndebug) log.warn "currentOperatingStates => $currentOperatingStates"
                if (logwarndebug) log.warn "allOk => $allOk"
                if (logwarndebug) log.warn "OperatingStateOk => $OperatingStateOk"
            ...
            if (!atomicState.setPointOverride && (thermModeNotNeed || forceCommand || (dontcheckthermstate && atomicState.dontcheckthermstateCount < 10)) && contactClosed) {
                ...
                if (forceCommand && OperatingStateOk) { if (enabledebug) log.debug "FORCING CMD TO DEVICE BECAUSE temperature difference is TOO HIGH" }
                if (forceCommand && !OperatingStateOk && !thermTempDiscrepancy) { if (enabledebug) log.debug "FORCING CMD TO DEVICE BECAUSE current operating state is INCONSISTENT" }
                ...
                if (enabledebug) log.debug "preferCooler ? $preferCooler && outside < preferCoolerLimitTemperature =  $outside < $preferCoolerLimitTemperature"
                if (enabledebug) log.debug "need != 'off' || forceCommand || (need == 'off' && (sensor || offrequiredbyuser)) = ${need != 'off' || forceCommand || (need == 'off' && (sensor || offrequiredbyuser))}"
                if (enabledebug) log.debug "need == 'cool' && preferCooler && (outside < preferCoolerLimitTemperature || !preferCoolerLimitTemperature) = ${need == 'cool' && preferCooler && (outside < preferCoolerLimitTemperature || !preferCoolerLimitTemperature)}"
                if (enabledebug) log.debug "thermMode = $thermMode"
                if (enabledebug) log.debug "need = $need"
                if (enabledebug) log.debug "thermModeNotNeed = ${thermModeNotNeed}"
                ...
                if ((need != "off" || !offrequiredbyuser) || forceCommand || (need == "off" && (sensor || offrequiredbyuser))) {
                    ...
                    if (enabledebug) log.debug "THERMOSTAT stays in $thermMode mode"
                }
            ...
            if (!atomicState.setPointOverride) {
                ...
                if ((need != "off" || !offrequiredbyuser || simpleModeIsActive()) && (currSpNotOk || thermModeNotNeed) && !thermTempDiscrepancy && contactClosed) {
                    ...
                    set_thermostat_target_ignore_setpoint(cmd, target, motionActive, "d54fdhj")
                    ...
                    if (currSpNotOk || thermModeNotNeed) {
                        ...
                        if (currSpNotOk || (thermModeNotNeed && need != "off")) {
                            ...
                            set_thermostat_target_ignore_setpoint(cmd, target, motionActive, "sljdfo")
            ...
            log.error "CONSISTENCY TESTS AND EMERGENCY ==> $e"
        }
    }
    else {
...
def powerConsistency(need, target, cmd, contactClosed, doorsContactsAreOpen, motionActive){
    try {
        if (pw) {
            ...
            {
                if (description) log.info "<i style=color:red;backgroundColor:red;></i> : pwLow = $pwLow refreshing $pw because power is $pwVal while it should be ${need == "off" ? "below 100 Watts":"above 100 Watts"}"
                pollPowerMeters()
            }
            ...
            if (!ignoreMode && timeIsUp && pwLow && (need != "off" || !offrequiredbyuser)) {
                ...
                if (cmd in ["setCoolingSetpoint", "setHeatingSetpoint", "setThermostatSetpoint"]) {
                    ...
                    set_thermostat_target_ignore_setpoint(cmd, target, motionActive, "resend time up")
                }
                else {
                    if (cmd.contains("off")) {
...
def fanCirculateManagement(need, target, contactClosed, motionActive){
...
def checkEfficiency(coolerNotEfficientEnough, boost, thermMode, need){
    ...
    if (enabledebug) log.debug "coolerNotEfficientEnough = $coolerNotEfficientEnough boost = $boost thermMode = $thermMode"
...
def virtualThermostat(need, target){
    ...
    if (heater || altThermostat || inAllHeatSourcesMode || altHeatExclusiveMode) {
        ...
        if (controlPowerConsumption && atomicState.lastNeed == "heat") {
            ...
            if (tooMuchPower) {
                if (tracedebug) log.trace "both $thermostat and $heater are using too much power but heater needs to stay on due to outside temperature being too low for the heat pump to remain efficient. $thermostat should be off"
            }
        ...
        {
            need = "off"
            if (logwarndebug) log.warn "$thermostat and $heater use too much power at the same time. Turning $heater off since $thermostat takesh precedence"
        }
...
def getTarget(simpleModeActive){
    ...
    if (enabledebug) debugFromList(debug)
...
def getNeed(target, simpleModeActive, inside, motionActive, origin){
    ...
    boolean doorsContactsAreOpen = doorsContactsAreOpen()
    boolean doorsOverride = doorsManagement ? doorsOverrideMotion : true // must return true by default if none due to "(!doorsContactsAreOpen && doorsOverrideMotion)"
...
def set_target(cmd, target, motionActive, origin){
    if (differentiateThermostatsHeatCool) {
        def simpleModeActive = simpleModeIsActive()     
        def need = getNeed(target, simpleModeActive, getInsideTemp(), motionActive, "set_target")   
        def neededThermostats = get_needed_thermosats(need)
...
def set_thermostat_target_ignore_setpoint(cmd, target, motionActive, origin){
    ...
    if (ignore && !inpowerSavingMode && !exceptForThermostatCool && !exceptForThermostatHeat) {
        ...
        set_target(cmd, target, motionActive, "dsfszd")
    }
}
...
def getInsideHumidity(){
    ...
    if (!optionalHumSensor) {
        ...
        {
            log.erro  formatText("NOR YOUR THERMOSTAT NOR YOUR OUTSIDE SENSOR SUPPORT HUMIDITY MEASUREMENT - PICK A DIFFERENT SENSOR IN YOUR SETTINGS", "black", "red")
        }
    }
    else {
...
boolean Active(){
    ...
    if (motionSensors) {
        ...
        if (inMotionMode) {
            ...
            log && tracedebug.trace("motion returns true because outside of motion modes")
        }
...
def disablelogging(){
    log.warn "debug if(enabledebug) log.debug disabled..."
