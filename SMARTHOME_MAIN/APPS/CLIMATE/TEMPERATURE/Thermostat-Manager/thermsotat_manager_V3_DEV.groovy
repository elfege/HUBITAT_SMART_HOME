...
def MainPage() {
    ...
    dynamicPage(pageProperties) {
        ...
        section("Thermostats"){
            ...
            input "set_to_fan_only_instead_off", "bool", title: "Set thermostats to 'fan_only' instead of 'off' (recommended to avoid mold formation)", submitOnChange:true
            if(fan_only){
                ...
                thermostats.each { thermostat -> 
                    ...
                    input "fan_speed_for${thermostat.displayName}", "enum", 
        ...
        section("Comfort Intelligence") {
            ...
            def use_ai_descr = useAI ? "(<b>AI enabled</b>) Disable AI management (not destructive)" : "(<b>AI disabled</b>) Enable AI management (beta)"
...
def pushableButtonHandler(evt){
    ...
    if(nightModeActive){
        logTrace formatText("NIGHT MODE ACTIVATED", "white", "orange")
...
def master(data){
   ...
   if(restriction().data.restricted_mode && data.calledBy != "appButtonHandler") return
    ...
    if (new Date().time - state.lastRun > lapse * 60000 || emergency){
...
def handleThermosats(motionActiveEvent=false, isButtonNightMode=false){
    ...
    allThermostats.each { thermostat -> 
        ...
        if (isButtonNightMode && !isButtonNightModeTherm) {
            logWarn "Keeping ${thermostat.displayName} off due to night/sleeping mode"
...
def setThermostatsMode(data){
    ...
    def cmdList = need == "fan_only" ? ["setThermostatFanMode", "setSpeed"] : ["setThermostatMode"] // we'll need both to set the thermostatFanMode(speed_value) and the thermostatMode('fan_only')
    
    def attribute = need == "fan_only" ? ["setThermostatFanMode", "setSpeed"] : ["setThermostatMode"] 
    ...
    if(device){
        ...
        if (need == "fan_only") {
            setThermostatsFanModesAndSpeed()
        } else if(need in ["heat", "cool"]){
            if (device.currentValue(attribute) != need
                device."${cmd}"(need)
            }
        }
    }
    else {
        logError "Device with ID ${data.deviceId} not found (setThermostatsMode)."
        return 
    }
}
def setThermostatsFanModesAndSpeed(thermostat, need, cmdList){
    // in fan_only mode, don't be in any of the active modes (or it'd trigger turbo in fan_only (which is an auto therm mode))

    if(thermostat in fan_only_thermostats) {
        if(set_to_fan_only_instead_off){
            logDebug "Setting ${device.displayName} to 'fan_only' before setting thermostatFanMode (fan speed)"

            def supportedFanModesJson = device.currentValue("supportedThermostatFanModes")
            def fanModes = new groovy.json.JsonSlurper().parseText(supportedFanModesJson)
            logDebug "------------Parsed thermostat Fan Modes: $fanModes (${fanModes.class})"

            def compatible_fan_speed = settings["fan_speed_for${device.displayName}"] ?: "medium" // medium is just a fallback here. Unlikely to happen. 

            logWarn "compatible value fan speed/fanMode used for $device.displayName: $compatible_fan_speed"  
            
            def fanMode = attribute in ["thermostatFanMode", "fanSpeed"] ? compatible_fan_speed : need 
            
            
                // we run through both setthermostatFanMode and setSpeed 
                for (cmd in cmdList){
                    if(cmd == "setSpeed" && device.hasCommand("setSpeed")){
                        if (device.currentValue('fanSpeed') != fanMode){
                            device.setSpeed(compatible_fan_speed) 
                        }
                    }
                    else if (device.currentValue(attribute) != fanMode){
                        device."${cmd}"(fanMode)
                    }
                }
                logTrace "${device.displayName} ${attribute} set to ${fanMode}"
            }
            else {
                logTrace "${device.displayName} ${attribute} <b>ALREADY</b> set to ${fanMode}"
            }
        
        // not for this device (!use_fan_only => it's the exception device)
        } else {
    ...
    } else if(!set_to_fan_only_instead_off && need == "off") {
        
            logTrace " Turning off ${device.displayName}."
            if(device.currentValue("thermostatMode") != "off") device.off()
    } else {
        logTrace "${thermostat.displayName}'s fan speed not managed." // user gets manual control over this mode. 
    }
...
def restriction(motionActiveEvent=false){
    ...
    def off_mode = set_to_fan_only_instead_off ? "fan_only" : set_to_auto_instead_of_off ? "auto" : "off"
        ...
        */ 
        ...
        if (currentIndoorTemp > (targetTemp + comfortRange.upperBound)) {
            if (naturalCoolingPotential.feasible) {
                ...
                result = use_auto_instead_off ? "auto" : set_to_fan_only_instead_off ? "fan_only" : "off"
            } else {
                ...
                if(changeMd){
                    ...
                    if (state.currentThermalEvent?.type == "cooling") {
                        recordThermalEvent([
            ...
            if (naturalHeatingPotential.feasible) {
                ...
                ********************************************************************************************/
                ...
                if(changeMd){
                    ...
                    if (state.currentThermalEvent?.type == "naturalHeating") {
                        recordThermalEvent([
                            endTemp: currentIndoorTemp,
                            isNewEvent: false
                        ])
        ...
        log.error "(app is FALLING BACK TO LEGACY ALGORITHM) - Exception in get_need() => $e "
        return fallback_need_eval(motionActiveEvent=motionActiveEvent, logs=true)
    }
    
    result = !result ?: (use_auto_instead_off ? "auto" : set_to_fan_only_instead_off ? "fan_only" : "off")
...
def assessNaturalThermalPotential(prediction, targetTemp, currentTemp, outdoorTemp, thermalThreshold, heatLoad, humidity, mode) {
    ...
    */
    ...
    if (state.currentThermalEvent) {
        ...
        if (state.currentThermalEvent?.type == "natural${mode.capitalize()}") {
            ...
            if (duration > maxDuration) {
                alwaysWarn "Natural ${mode} attempt exceeded maximum duration"
                recordThermalEvent([
                    endTemp: currentTemp,
                    isNewEvent: false
                ])
                ...
                //update the prediction value as if we got here it means the prediction didn't work
    ...
    // Prefer natural methods if we recently changed modes
   ...
   // Create new event if we don't have one already set 
    if ((!recordingInProgress?.isNewEvent || recordingInProgress.type != "natural${mode.capitalize()}")) {
        recordThermalEvent([
    ...
    alwaysWarn "assessNaturalThermalPotential ($mode): $result"
