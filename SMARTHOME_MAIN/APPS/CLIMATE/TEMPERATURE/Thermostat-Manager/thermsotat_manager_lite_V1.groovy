...
def initialize(){
    ...
    state.values = [
        ...
        "currentOutdoorTemp": outsideTemp.currentValue("temperature") as BigDecimal,
        "currentSetpoint": dimmer.currentValue("level") as BigDecimal
    ]
}
...
def ensureProperSetpoint(){
    log.debug """
    state.values.currentSetpoint = $state.values.currentSetpoint
    state.userDefinedSetpoint = $state.values.currentSetpoint
    """
    if(state.values.currentSetpoint != state.userDefinedSetpoint)
    {
        log.debug "restoring required setpoint: $state.userDefinedSetpoint = evt.value as BigDecimal to state.values"
        state.values.currentSetpoint = state.userDefinedSetpoint as BigDecimal
    }
}
def get_need(){
    try {
        def currentSetpoint = dimmer.currentValue("level") as BigDecimal
        def currentMode = state.values.currentMode
        def currentIndoorTemp = state.values.currentIndoorTemp as BigDecimal
        def currentOutdoorTemp = state.values.currentOutdoorTemp as BigDecimal
        ...
        def coolDiff = location.mode in powerSavingMode ? 8.0 : 2.0
        def heatDiff = location.mode in powerSavingMode ? 8.0 : -2.0
        ...
        // Determine if temperature deviation is severe (4 degrees beyond normal threshold)
        def highAmplitude = location.mode in powerSavingMode ? false : indoorToSetpointDiff >= (coolDiff + 4.0)
        def lowAmplitude = location.mode in powerSavingMode ? false : indoorToSetpointDiff <= (heatDiff - 4.0)
        ...
        def changeMode = { String newMode ->
            ...
            def isActiveMode = { mode -> mode in ["heat", "cool"] }
            def isInactiveMode = { mode -> 
                mode in [fan_only ? "fan_only" : "off", "auto"]
            }
            
            // Only apply delay when switching between heat and cool
        ...
        if (indoorToSetpointDiff >= coolDiff) {
            if (currentOutdoorTemp >= currentIndoorTemp || highAmplitude) {
                ...
                return changeMode(fan_only ? "fan_only" : "off")
            }
        } 
        ...
        else if (Math.abs(indoorToSetpointDiff) < 1.0) {
            log.debug "Temperature within 1° of setpoint - maintaining"
            return changeMode("auto")
        }
        else {
            log.debug "No extreme conditions detected - setting to off"
            return changeMode(fan_only ? "fan_only" : "off")
        }
    }
    catch (Exception e){
