...
def initializeLife360Connection() {
    ...
    def url = "https://api-cloudfront.life360.com/v4/users/premium"
    ...
    try {
        ...
        // httpPost(uri: url, body: postBody, headers: ["Authorization": "Basic NWI1NTYyNGYtZjJlOC00MTA1LWFiMjMtYTY5ZTBjMjBmMTMz"]) { response ->
            httpPost(uri: url, body: postBody, headers: 
                [
        ...
        def circles = resultCircles.data.circles
          ...
          input "circle", "enum", multiple: false, required: true, title: "Life360 Circle", options: circles.collectEntries{ [it.id, it.name] }, submitOnChange: true
            ...
            def result = null
                ...
                httpGet(uri: url, headers: ["Accept": "application/json", "cache-control": "no-cache", "user-agent": "com.life360.android.safetymapd/KOKO/23.50.0 android/13", "Authorization": "Bearer ${state.life360AccessToken}", timeout: 30]) {
                    response ->
                    result = response
                }
...
def createCircleSubscription() {
    ...
    def deleteUrl = "https://api-cloudfront.life360.com/v3/circles/${state.circle}/webhook.json"
        try {
            ...
            httpDelete(uri: deleteUrl, headers: ["Accept": "application/json", "cache-control": "no-cache", "user-agent": "com.life360.android.safetymapd/KOKO/23.50.0 android/13", "Authorization": "Bearer ${state.life360AccessToken}"]) {
                response ->
                result = response
            }
        }
    ...
    def result = null
        try {
            httpPost(uri: url, body: postBody, headers: ["Accept": "application/json", "cache-control": "no-cache", "user-agent": "com.life360.android.safetymapd/KOKO/23.50.0 android/13", "Authorization": "Bearer ${state.life360AccessToken}"]) {
                response ->
                result = response
            }
        } catch (e) {
...
def placeEventHandler() {
    ...
    def postUrl = "https://api-cloudfront.life360.com/v3/circles/${circleId}/members/${memberId}/request.json"
        ...
        try {
            httpPost(uri: postUrl, body: ["type": "location"], headers: ["Accept": "application/json", "cache-control": "no-cache", "user-agent": "com.life360.android.safetymapd/KOKO/23.50.0 android/13", "Authorization": "Bearer ${state.life360AccessToken}"]) {
                response ->
