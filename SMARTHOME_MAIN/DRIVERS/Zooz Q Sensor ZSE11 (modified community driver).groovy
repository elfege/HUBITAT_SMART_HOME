...
void refresh() {
   ...
   log.debug "Device is DC-powered; running refresh commands now (modified by elfege to always believe it's on dc power source!)"
...
void updated() {
   ...
   log.debug("power source always set to dc by elfege")
   sendEvent(name: "powerSource", value: "dc", descriptionText: "${device.displayName} power source is dc")
