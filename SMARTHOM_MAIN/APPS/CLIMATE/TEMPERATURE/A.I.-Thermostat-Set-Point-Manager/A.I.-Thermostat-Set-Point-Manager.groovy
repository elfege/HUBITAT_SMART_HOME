    ...
    dynamicPage(pageProperties) {
        section()
        {
            if(contact || windows)
            {
                ...
                input "fanCirculateSimpleModeOnly", "bool", title:"Run fan circulation only in simple mode", submitOnChange:true, defaultValue:false
                ...
                if(fancirculateAlways)
                {
                    ...
                    input "alwaysButNotWhenPowerSaving", "bool", title:"Fan circulation must stop when no motion or in away mode", defaultValue:false
                }
                ...
                input "splitACcustomFirmWare", "bool", title: "I'm the developper of this app (please ignore this option uniquely designed for my config)", defaultValue:false
...
def dimmerHandler(evt){
    ...
    if(!atomicState.paused){
        ...
        {
            logging "dimmer value set by this app"
        }
...
"""
        ...
        {
            log.trace "not changing fan mode because thermostat is currently ${need}ing"   
        }
        else
        {
                ...
                {                
                    if((!OperatingStateOk || now() - atomicState.lastSetTime > 5 * 60 * 1000) || need == "off" || forceCommand)
                    {
                        ...
                        if(need == "cool" && preferCooler && (outside < preferCoolerLimitTemperature || !preferCoolerLimitTemperature))
                        {
                            ...
                            if(thermMode != need && (!fancirculateAlways || need != "off")){
...
*/
    ...
    if(cooler)
    {
        ...
        log.trace "coolerCurVal = ${coolerCurVal} ${powerCapable ? "powerValue = $powerValue" : ""} powerDiscrepancy = $powerDiscrepancy"


        if(need == "cool"){
            if(coolerCurVal != "on" || powerDiscrepancy)
            {
                log.trace "${powerDiscrepancy ? "POWER DISCREPANCY: turning on $cooler AGAIN" : "turning on $cooler"}"
                cooler?.on()
        ...
        if(need == "off"){
            ...
            {
                log.trace "${powerDiscrepancy ? "POWER DISCREPANCY: turning off $cooler AGAIN" : "turning off $cooler"}"
                cooler?.off()
...
def getTarget(simpleModeActive){
    ...
    {
        target = getAutoVal()
        log.trace "getAutoVal() returned $target"
    }
    else
    {
...
"""
    ...
    if(simpleModeActive)
    {
        ...
        {
            log.trace "some doors are open: simple mode trigger mode ignored at user's request"
        }
        ...
        {       
            ...
            log.trace "target temperature ${substract ? "(specialSubstraction)":"(specialTemp)"} is: $target and last recorded temperature is ${inside}"
            return target // END due to simple mode trigger mode
        }
        else
        {
            log.trace "simpleModeActive using default target temp: $target | last recorded temperature is ${inside}"
            return target // return the default value
        }
    } 
    log.trace "target temperature is: $target and current temperature is ${inside} (swing = $atomicState.swing) thermostat state is: ${thermostat.currentValue("thermostatMode")}"
...
"""
    ...
    else if(!INpwSavingMode && contactClosed && motionActive)
    {
        ...
        {
            if(fancirculateAlways && splitACcustomFirmWare) // this option only applies to the developper particular config, ignore it
            {
                need0 = "off"
                need1 = "off"
                logging("need set to OFF")
...
"""        
        ...
        if(inside > criticalhot || fancirculateAllways)
        {
            ...
            if(!contactClosed) // if contacts open then just fan circulate
            {
                ...
                if(fancirculate)
                {
                    boolean thermInOperatingMode = thermMode in ["cool", "heat"]
                    if(!thermInOperatingMode && thermostat.currentValue("thermostatFanMode") != "on") // no need to send this command when cooling or heating
                    {
                        if(!motionActive && alwaysButNotWhenPowerSaving)
                        {
                            if(thermMode != "off")
                        ...
                        {
                            log.trace "fanOn() criticalhot or fancirculateAllwas ($fancirculateAlways)"
                            thermostat.setThermostatFanMode("on")
...
def getAutoVal(){
    ...
    log.warn "variation amplitude = $variation | absolute need (auto method, not from getNeed()) is $need outside: $outside "
...
def getVariationAmplitude(outside, need){
    ...
    log.debug "atomicState.db //= ${atomicState.db.sort{it.value.toInteger()}}"
    ...
    log.trace "minD = $minD, keySearch = $keySearch"
...
"""
    log.info "db variation amplitude = ${y.toInteger()}"
    return y.toInteger()
...
def getLastMotionEvents(Dtime, testType){
    ...
    if(motionSensors.any{it -> it.currentValue("motion") == "active"})
    {
        ...
        events = 10 // this is not a boolean method but called by a through the definition of a boolean variable
