...
"""
    ...
    logging "****** target = $target************ dimmerLevel = ${dimmer?.currentValue("level")}"
    ...
    if(simpleModeActive && !simpleModeSimplyIgnoresMotion)
    {
        ...
        {       
            ...
            logtrace "target temperature ${substract ? "(specialSubstraction)":"(specialTemp)"} is: $target and last recorded temperature is ${inside}"
...
--**--"""
    ...
    boolean needCool = !simpleModeActive || (simpleModeActive && simpleModeSimplyIgnoresMotion) ? (inWindowsModes ? outsideTemperature >= outsideThres && inside >= target + swing : outsideTemperature >= outsideThres && inside >= target + swing) : outsideTemperature >= outsideThres + 5 && inside >= target + swing
...
""", "white", "blue")
    ...
    if(need1 == "cool") atomicState.lastTimeCool = now() // keep track of time to avoid oscillations between cool / heat during shoulder season
    if(need1 == "heat") atomicState.lastTimeHeat = now() // same idea
    ...
    descriptionText "need: $need | atomicState.lastNeed = $atomicState.lastNeed"
    return need
