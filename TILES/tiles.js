            ...
            allDevices.forEach((e, index) => {
                ...
                if (notAButton && (isLight || isLock || isSwitchLevel)) {
                    ...
                    if (isSwitchLevel) {
                        ...
                        $("#dimmers").append(
                            $("<div>").attr("id", id_html)
                                .addClass("float-left mt-5 mr-2 text-center dimmerObject")
                                .roundSlider().attr({
                                    "id": id_html,
                                    "data-id_From_Hub_level": id_From_Hub_level, // every dimmer is also a switch, so we need a different data attr here. 
                                    "data-state": ""
                                })
                        ...
                        $(`#${id_html}`).prepend($("<span>")
                            .attr({
                                "id": idspan,
                                "data-id_From_Hub": id_From_Hub // we want the switch on/off value here
                            })
                            .addClass("spanDimmer")
                            .text(label))
                        ...
                        $(`#${id_html}`).on("change", function (evt) {
                            ...
                            // console.log("device id = ", id_From_Hub_level)
                    ...
                    if (isLock) {
                        ...
                        /************************CREATE LOCKS ********************************/
                        $("#lockContainer").append($("<div>").addClass("row")).append(
                            $("<a>").addClass("btn btn-outline-primary btn-block text-white").attr({
                                "id": `${id_html}`,
                                "data-id_From_Hub": id_From_Hub,
                                "data-state": ""
                        ...
                        $(`#${id_html}`).on("click", () => {
                            ...
                            axios.get(`http://${ip}/apps/api/${appNumber}/devices/${id_From_Hub}?access_token=d6699bb3-0d13-48d1-ab5d-8cc583efa76c`)
                                .then(resp => {
                                    ...
                                    console.log(`${data.label} is ${state}. ${state === "locked" ? "unlocking" : "locking"}`)
                                    const cmd = state === "locked" ? "unlock" : "lock"
...
function updateDeviceState(id_From_Hub, id_html, type, notAButton, value) {
    axios.get(`http://${ip}/apps/api/${appNumber}/devices/${id_From_Hub}?access_token=d6699bb3-0d13-48d1-ab5d-8cc583efa76c`)
        .then(resp => {
            const data = resp.data
            const state = data.attributes.find((val) => {
            ...
            if (type === "switch&Level") {
                let color = value > 1 && state === "on" ? "yellow" : "white"
