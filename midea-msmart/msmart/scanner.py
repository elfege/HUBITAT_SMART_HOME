...
class scandevice:
    ...
    async def support_testv3(self, account, password):
        ...
        # Skip token retrieval and authentication for local discovery
        if not account or not password:
            _LOGGER.info("Skipping cloud authentication for local discovery.")
            return _device
        ...
        for udpid in [get_udpid(self.id.to_bytes(6, 'little')), get_udpid(self.id.to_bytes(6, 'big'))]:
            ...
            # Skip authentication if token and key are not retrieved
            if token is None or key is None:
                _LOGGER.info(f"Skipping device authentication for {self.ip} (local discovery).")
                return _device
